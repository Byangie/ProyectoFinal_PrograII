package com.example.patitasvivas.Presentacion.PantallaInicial
...
@Composable
fun MostrarMascotas(auth: FirebaseAuth) {
        ...
        petsCollection.whereEqualTo("IdUsuario", auth.currentUser?.uid.toString()).get()
            .addOnSuccessListener { documents ->
                if (!documents.isEmpty) {
                    ...
                    mascotas = documents.map { it.data }.filter { mascota -> mascota["Estado"] as? String != "Adopcion"}
                } else {
    ...
    Column(
        ...
        if (mascotas == null) {
            ...
            Text(
                text = "No tienes mascotas registradas. ¡Registra una mascota!",
                color = Color.White,
        ...
        Column(
            ...
            Button(onClick = {
                ...
                petsCollection.document(Idmascota)
                    .update("Estado", "Adopcion")
                    .addOnSuccessListener {
                        // Mostrar mensaje de confirmación en un Snackbar
                        scope.launch {
                            snackbarHostState.showSnackbar("Mascota en adopción")
                        }
                    ...
                    .addOnFailureListener {
                        ...
                        scope.launch {
                            snackbarHostState.showSnackbar("Error al poner en adopción")
                        }
                    }
            },
                modifier = Modifier.fillMaxWidth().height(48.dp).padding(horizontal = 32.dp),
                colors = ButtonDefaults.buttonColors(containerColor = Green)) {
                Text("Dar en Adopción") // Texto del botón.
            }
        }
    }
}
